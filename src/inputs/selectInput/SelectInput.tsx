...
export default class SelectInput extends React.Component<SelectInputProps, SelectInputState> {
        ...
        if(this.props.type === 'multi'){
            window.addEventListener('click', (event:any) => {
                ...
                event.path.forEach((value:any)=>{
                    if(value.className !== undefined && value.className !== "" && value.className.indexOf("mg-select-input") !== -1 && value.className.indexOf('select-' + this.state.randomId) !== -1){
                        control = true;
                    }
                });
    ...
    render(){
        ...
        return <div className={`mg-select-input select-${this.state.randomId} mg-input`} onClick={() => { if(this.props.type !== "single") { this.inputFocus();} }}>
    ...
    singleSelectResult(){
        ...
        return <select className={`form-control mg-select ${this.props.className}`} value={activeId} name={this.props.name} onChange={(e)=>{
